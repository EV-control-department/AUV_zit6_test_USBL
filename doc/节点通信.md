...
- 角度单位：弧度（rad）；线速度：米每秒（m/s）；位置：米（m）；推力或力：牛顿（N），嵌入式内部也可使用归一化推力 `[-1, 1]` 表示最大推力百分比，规范中会标注。

### 坐标系定义

本系统使用两套坐标系，**两者轴向定义不同**，注意区分：

| 坐标系 | 约定 | X 轴 | Y 轴 | Z 轴 | 原点 |
|---|---|---|---|---|---|
| **机体系（Body Frame）** | FRD | 前（机头） | 右 | 下（向海底为正） | AUV 自身重心，随艇旋转移动 |
| **世界系（World Frame）** | NED | 北 | 东 | 下（向海底为正） | 任务起始点 / INS 上电归零点，固定于空间 |

> **Yaw（偏航）约定（两套坐标系相同）**：绕 Z 轴旋转，顺时针为正（从上往下看，即北偏东为正）。
>
> **⚠️ 注意**：机体系 X 是「前」，世界系 X 是「北」，两者只有在 AUV 正北朝向时才重合。混用会导致控制指令方向错误。

#### 使用场景说明

- **机体系（Body Frame）**：
  - 适用于**速度控制**（如"向前游 0.5 m/s"）、**推力分配**（直接映射到推进器坐标）。
  - 话题 `/zit6/state/vel`、`/zit6/state/thr` 均以机体系表示。
  - 在 `ZitSetpoint` 中：将 `control_key` 的 **Bit4（0x10）置1** 表示目标为机体系。

- **世界系（NED）**：
  - 适用于**位置控制**（如"保持在起始点北偏 3m 处"）、**轨迹规划**。
  - 话题 `/zit6/state/pos` 发布的 `[x, y, z, yaw]` 含义为：**北向位置增量（m）、东向位置增量（m）、深度（m，向下为正）、航向角（rad）**，均相对于 INS 上电归零点。
  - 在 `ZitSetpoint` 中：**Bit4 = 0**（默认）即为世界系（NED）目标，x/y/z 分别对应北/东/深度。

#### 坐标系转换关系

NED 世界系与 FRD 机体系通过当前航向角 `ψ`（北偏东为正，rad）转换：

```
# FRD 机体系速度 → NED 世界系速度
v_north =  vx_body * cos(ψ) - vy_body * sin(ψ)
v_east  =  vx_body * sin(ψ) + vy_body * cos(ψ)
v_down  =  vz_body   # Z 轴与航向无关，深度直接对应
```

> `/zit6/state/vel` 发布的是机体系速度（FRD），`/zit6/state/pos` 发布的是世界系位置（NED），固件内部做了此变换。

> **注意**：增量模式（`control_key` Bit5=1）下，机体系增量指令会由固件自动转换到世界系后再累加到目标位置，上位机无需手动做坐标变换。
